...
The calendar sync pipeline relies on the following Google-specific variables in
addition to the OAuth credentials:

- `GOOGLE_CALENDAR_WEBHOOK_URL` – publicly accessible HTTPS endpoint that Google
  Calendar will call for push notifications. Point this to
  `<backend-url>/integrations/google/webhook/calendar`.
